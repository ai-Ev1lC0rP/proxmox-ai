...
async def start():
    ...
    # Welcome message
    await cl.Message(
        content="Welcome to the Proxmox Server Whisperer! 🧙‍♂️ I can tame your wild servers and decode their mysterious behaviors.",
        author="Assistant"
    ).send()
    
    # Message to explain available modes
    await cl.Message(
        content="Available modes:\n" + 
                "- Server Whisperer: Direct assistance with Proxmox tasks\n" +
                "- Solution Comparison: Compare different approaches\n" + 
                "- Mystical Web Search: Gather knowledge from the web\n" + 
                "- Proxmox Guru: Deep expertise on Proxmox topics",
        author="Assistant" 
    ).send()
    
    # Message about temperature and token settings
    await cl.Message(
        content=f"Using default settings: temperature={MODEL_PARAMS['temperature']}, " +
                f"top_p={MODEL_PARAMS['top_p']}, max_tokens={MODEL_PARAMS['max_tokens']}",
        author="Assistant"
    ).send()
